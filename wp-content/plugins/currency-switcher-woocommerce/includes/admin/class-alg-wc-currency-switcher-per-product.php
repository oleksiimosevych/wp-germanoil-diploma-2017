...
class Alg_WC_Currency_Switcher_Per_Product_Settings {
	...
	function get_meta_box_options() {
		...
		foreach ( $products as $product_id => $desc ) {
			for ( $i = 1; $i <= $total_number; $i++ ) {
				if ( 'yes' === get_option( 'alg_currency_switcher_currency_enabled_' . $i, 'yes' ) ) {
					$currency_code = get_option( 'alg_currency_switcher_currency_' . $i, $currency_from );
					if ( $currency_code != $currency_from ) {
						$currencies = array_merge( $currencies, array(
							array(
								'name'       => 'alg_currency_switcher_per_product_regular_price_' . $currency_code . '_' . $product_id,
								'default'    => '',
								'type'       => 'price',
								'title'      => '[' . $currency_code . '] ' . __( 'Regular Price', 'currency-switcher-woocommerce' ),
								'desc'       => $desc,
								'product_id' => $product_id,
								'meta_name'  => '_' . 'alg_currency_switcher_per_product_regular_price_' . $currency_code,
							),
							array(
								'name'       => 'alg_currency_switcher_per_product_sale_price_' . $currency_code . '_' . $product_id,
								'default'    => '',
								'type'       => 'price',
								'title'      => '[' . $currency_code . '] ' . __( 'Sale Price', 'currency-switcher-woocommerce' ),
								'desc'       => $desc,
								'product_id' => $product_id,
								'meta_name'  => '_' . 'alg_currency_switcher_per_product_sale_price_' . $currency_code,
							),
						) );
					}
				}
		...
		/* return array_merge( array(
			array(
				'name'       => 'alg_currency_switcher_per_product_settings_enabled',
				'default'    => 'no',
				'type'       => 'select',
				'options'    => array(
					'yes' => __( 'Yes', 'currency-switcher-woocommerce' ),
					'no'  => __( 'No', 'currency-switcher-woocommerce' ),
				),
				'title'      => __( 'Enabled', 'currency-switcher-woocommerce' ),
			),
		), $currencies ); */
	}
